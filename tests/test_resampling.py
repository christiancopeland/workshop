...
class AudioStream:
    ...
    def __init__(self, 
        ...
        Note: If hardware_sample_rate differs from sample_rate, automatic
              resampling will convert hardware rate to target rate.
        """
    ...
    def _resample(self, audio_hw: np.ndarray) -> np.ndarray:
        """
        Downsample audio from hardware rate to target rate.
        
        Uses linear interpolation to guarantee EXACTLY frame_size samples output.
        This ensures VAD always receives exactly 512 samples.
        
        Args:
            audio_hw: Audio at hardware sample rate
            
        Returns:
            Audio at target sample rate with EXACTLY frame_size samples
        """
        if len(audio_hw) == 0:
            return np.array([], dtype=np.float32)
        
        # Generate exactly frame_size evenly-spaced sample points
        # This guarantees output length = frame_size
        indices = np.linspace(0, len(audio_hw) - 1, self.frame_size)
        
        # Interpolate to get values at those points
        resampled = np.interp(indices, np.arange(len(audio_hw)), audio_hw)
